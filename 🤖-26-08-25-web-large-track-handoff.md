# 🤖 웹 트랙 W — **W0c·W1a 완료(실측까지) · 다음 = W1b 웹 본체**

> **콜드 세션 시작점.** 사람이 *"뭐 하고 있었어? 이어서 하자"* 라고 물으면 **이 문서로 답한다.**
> 상태 정본은 [`ROADMAP.md`](ROADMAP.md) Now의 「▶ 재개 지점」 — 충돌하면 ROADMAP이 이긴다.
> **최종 갱신 2026-09-02 (W1a 종결 — 배포·실기기 실측·`main` 병합까지 끝).** 선행 핸드오프·규모 판정 브리프는 2026-08-25 서류 정돈에서 삭제했다(판정 결과는 §1·§2에 흡수).

**한 줄**: <https://devetym.com> 이 라이브다. **W0c·W1a가 둘 다 프로덕션에 들어갔다 (2026-09-01 · 09-02).** 650 정본이 D1에 있고, 프록시는 웹/앱을 Origin으로 갈라 각자의 캡을 쓰며 Turnstile이 켜져 있다. 프롬프트 정본은 워커로 옮겨졌다. **실기기 실측도 통과했다** — `bikeshedding` 생성이 정상 성공했고 저장된 행의 `prompt_version`이 이전 전과 **같은 태그**(`v2-pathA:956ba44a7c48`)다. **다음 한 걸음은 W1b 웹 본체** — 650장·검색·AI는 아직 한 줄도 없다.

---

## 0. 새 세션이 *"이어서 하자"* 를 들었을 때 — **읽고 바로 이 말을 하면 된다**

> **W0c·W1a 둘 다 끝났다 — 프로덕션 적용과 실측까지.** 남은 사람 게이트는 없다.
> **다음은 W1b 웹 본체**(650장 SSG + 검색 + AI 폴백). Turnstile site key는 §4에 있다.
> 문제가 생기면 롤백 = `cd ~/devetym-proxy && npx wrangler rollback` (직전 `d0504c73`).
