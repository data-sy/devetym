...
**한 줄**: <https://devetym.com> 이 라이브다. **W0c가 끝났다 (2026-08-26 ~ 09-01) — 650개 어원 정본이 프로덕션 D1에 들어갔고 실 앱 경로에서 한글 별칭까지 도달한다.** 프로덕션 실측 = entries **671** · authored **650** · aliases **1,304** · 키 1,975개 전부 N1. **다음 한 걸음은 W1a 프록시 하드닝**이고, 그 전에 정리 하나가 남았다 — 🙋 **`sandbox/w0c-d1-seeding` 브랜치 2개 병합·푸시**([`w0c-sandbox-roadmap.md`](w0c-sandbox-roadmap.md) §8). **웹 본체(650장·검색·AI)는 아직 한 줄도 없다.**
...
**① 🙋 브랜치 병합·푸시 〔사람〕** — `sandbox/w0c-d1-seeding` 이 `devetym`·`devetym-proxy`
양쪽에 있다. 병합 시 [`w0c-sandbox-roadmap.md`](w0c-sandbox-roadmap.md) **§8 절차**를 따르고
**같은 커밋에서 그 파일을 삭제한다**(남기면 stale 정본이 된다).
