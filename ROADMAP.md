...
- **M6 · Compose UI (6화면) 착수 준비** — M5 완료(아래 Done). 다음은 `feat/m6-...`를 `feat/m5-viewmodel` 위에 **스택** 분기(브랜치 보존). 프로세스: M6 스펙 슬라이스 저작 → 적대 비준 → (게이트 완화) Claude eyes-open 수용 → 구현. 6화면(검색·상세·북마크·히스토리·설정·온보딩) Composable + `collectAsStateWithLifecycle` + 네비게이션 + 테마/디자인 토큰. **⚠️ 디자인 상속**: 토큰·폰트·아이콘·와이어프레임은 `~/dev-etymology/docs/design`+`Fonts`+`Features`(메모리 [ios-design-assets-inheritable]). **⚠️ M5 이월 마감**: DR-4/DR5-2(상세 북마크 현재값 상태 소스 + 쓰기 유실창 — 확정 피드백과 함께 닫기), DR5-3(history limit 구체값), 로딩 애니메이션(iOS LoadingPhase)·문구(오프라인 구분). **⚠️ 검증 천장**: green이 컴파일·조립까지고 **시각 검증은 실기기 필요** — 그 지점을 「코드 완료·실기기 검증 필요」로 정직히 라벨.
