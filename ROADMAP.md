...
- **M7 · Koin 배선 + 앱 셸 착수 준비** — M6 완료(아래 Done). 다음은 `feat/m7-...`를 `feat/m6-compose-ui` 위에 **스택** 분기(브랜치 보존). 프로세스: M7 스펙 슬라이스 저작 → 적대 비준 → (게이트 완화) Claude eyes-open 수용 → 구현. Koin 모듈(repository·store·api·analytics·seam actual·ViewModel `viewModel{}`)·`AppDependencies` 실구현·앱 셸(`MainActivity`/iOS 진입점)이 M0 Greeting 대신 `AppRoot` 그리도록 연결·온보딩 게이트. **⚠️ M6 이월 마감**: **DR5-2 쓰기 내구성**(`toggleBookmark`를 앱/repository 스코프 또는 `NonCancellable`로 내구화 + 확정 피드백), **DR-2 단일-writer Mutex 강제**(M4/M5 이월 — `TermRepository`를 `single`로 배선하는 게이트 + 강제 실측). **⚠️ 검증 천장 지속**: 셸 연결 후에도 시각·상호작용은 실기기 필요.
