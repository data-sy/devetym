...
- **이관·자기완결화 트랙 · dev-etymology → devetym (진행 중 — 2026-07-10 착수, `feat/m9-release-verification` 위 작업).** 순수 [AI] 이관분(pages.yml·Scripts·docs/db-expand)은 **`315ea55` 커밋**. **✅ WU-2·3·5 완료(2026-07-10)** — 순수 [AI] 3건. **✅ WU-6 스윕 완료(2026-07-10)** — 전수 대조 원장 산출 + 자기완결성 확증(dev-etymology 런타임·빌드·CI·자산 의존 **0**) + 안전 이관 2건(지원 이메일 정합 버그·LICENSE). 잔여: WU-1(Pages·사람 활성화)·WU-4(Sentry·방침 사인오프)·WU-7(폐기, 사람 게이트). devetym이 병렬 원본 repo `~/dev-etymology`(원본 네이티브 iOS 앱 + 문서·스크립트·CI) **의존 없이 자기완결**이 되게 이관하고, 스윕 전수 완료 시 dev-etymology를 **폐기**한다. **이관 = 내용 이관 + 설계 이관**(거기서 쓰던 방식을 여기서도 쓰게). 소유 태그 규율은 M9와 동일(`[AI]`·`[AI→사람]`·`[사람→AI]`·`[사람]`). ⚠️ **specs·ADR·데이터/거버넌스면 이관은 승인 게이트**(자동수정 금지 — 발견+제안+승인). **작업단위(WU) 계획·결정 원장 정본 = [`docs/handoff/26-07-10-selfcontained-migration-plan.md`](docs/handoff/26-07-10-selfcontained-migration-plan.md)** — 독립 세션 단위로 끊은 WU-1~12 + 확정 결정(D1~D6: 크래시 SDK=Sentry KMP·ai-quality→ADR-0007 승인 등). 세션 워킹 체크리스트 = [`26-07-10-to-do-list.md`](26-07-10-to-do-list.md)(폐기가능).
  ...
  - ✅ **[이관][AI] db-expand·prompt-probe 파이프라인 (WU-2 완료 2026-07-10)** — `Scripts/{db-expand, generate_db.py, prompt-probe}`·`docs/db-expand/` 이관 검증: 경로/상대참조를 devetym 기준으로 정합(번들 DB=`shared/…/composeResources/files/terms.json`, 모델상수=`Constants.kt`), dry 스모크 green(`generate_db.py --validate-only` 650개 통과·`prompts.verify` sanity 통과). claude.ai 정액 수동 경로 유지(API 종량 회피). rounds/archive는 불변 히스토리로 보존. 구 백로그 #4.
  - ✅ **[이관][사람→AI] AI 품질 문서 → ADR-0007 (WU-3 완료 2026-07-10·D3 승인)** — `docs/ai-quality/`(6종 + 신규 README 인덱스) 이관, **[ADR-0007](docs/adr/0007-ai-prompt-quality.md)** 신설(시스템 프롬프트 버전 v1+v2 Path A·3분기·품질 게이트·미채택 closing/selfcheck v3 보류 근거 락). iOS 검증본이 `commonMain`(`ClaudePrompt.kt`/`ClaudeDto.kt`)에 계승됨을 코드로 확증. architecture 참조 반영. 정직 갭 기록: 프롬프트 불변식 회귀 테스트 미포팅(출시후 선택). 구 백로그 #3.
  ...
  - ✅ **[대조][AI] launch-prep 잔여 (WU-5 완료 2026-07-10)** — `docs/launch-prep/`(6종)+`e2e-checklist.md` ↔ `docs/release/` 대조. 미승계 마케팅 델타만 이관: 스크린샷 캡션 5종→`m9-screenshot-capture-handoff.md` §1b, 대안 ASO 카피·후크형 설명→`m9-store-metadata-draft.md` 부록 A, Kids Category 금지 caution→§4. **개인정보 라벨 콘텐츠는 이관 금지**(devetym 미수집 정합 회귀 방지). 파일별 원장 = [WU-5 ledger](docs/handoff/26-07-10-wu5-launch-prep-ledger.md). 구 백로그(launch-prep 대조).
  - ✅ **[스윕][사람→AI] DevEtym/ 네이티브 iOS 전수 (WU-6 완료 2026-07-10)** — `~/dev-etymology/DevEtym/` Features·Services·Utils·Models·Assets.xcassets·Resources + `.claude/`·`LICENSE`·`README`·`docs/`·CI·`.gitignore` 전수 대조(5슬라이스 병렬). **파일별 이관/폐기 원장 = [WU-6 ledger](docs/handoff/26-07-10-wu6-native-sweep-ledger.md).** 핵심: **자기완결성 확증** — dev-etymology 런타임·빌드·CI·자산 커플링 **0**(남은 문자열 2건은 순수 출처 주석). 안전 이관 2건 실행(5축 green): **🐛 지원 이메일 정합 버그**(인앱 `data.sy.2@gmail`→방침·스토어와 일치하는 `oddmuffinstudio@gmail`, `Constants.supportEmail` 중앙화)·**LICENSE**(MIT 누락 이관). **잔여(개별 승인/연계)**: UX 패리티 포트(Detail 3단 로딩·History 확인 다이얼로그·이중언어 배지) / 프라이버시·거버넌스 얽힘(온보딩 동의·"내 식별자 보기"→WU-4 연계, 인앱 방침 URL→WU-1 연계, CLAUDE.md·commands 신규저술) / iOS Xcode 자산(AppIcon·다크변형·UILaunchScreen→M9 게이트). 구 백로그 #12. 〔선례: 아이콘 SVG 4종 이관 완료(2026-07-05)〕
  - **[폐기][사람] dev-etymology 폐기 결정** — WU-6 스윕상 코드/빌드/CI/자산 의존 0 확인됨. 단 원본에만 있는 자산·근거(iOS appiconset PNG·icon-dark·LaunchLogo·typography-review·design-followup·`.claude` 원본)를 뽑아둘지 결정(WU-6 ledger §3.2~3.3·§4) 후 폐기. ⚠️ **devetym 브랜치 보존 규율과 별개**(원본 repo 폐기는 사람 최종 확인). 구 백로그 #12.
