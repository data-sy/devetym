...
- **M5 · ViewModel + StateFlow 착수 준비** — M4 완료(아래 Done). 다음은 `feat/m5-...`를 `feat/m4-repository` 위에 **스택** 분기(브랜치 보존). 프로세스: M5 스펙 슬라이스 저작 → 적대 비준 → (게이트 완화) Claude eyes-open 수용 → 구현. 화면 상태를 `sealed` + `StateFlow<UiState>`로 노출(spec 3-3·architecture §4.5), `TermRepository`만 주입, pinned/refresh 상태 노출. ⚠️ **M4 단일-writer 계약 상속**(같은 `normalizeKeyword` fetch/refresh/toggleBookmark 직렬화 — M4 §3-1).
