...
- **M0(KMP 골격) 완료 — green 루프 확립.** `shared + androidApp + iosApp` 골격이 서고 세 오라클 통과:
  `:shared:testDebugUnitTest` · `:androidApp:assembleDebug` · `:shared:linkDebugFrameworkIosSimulatorArm64`(SKIE 포함).
  - 남은 셸 단계: **iOS `iosApp.xcodeproj` 생성**(전체 시뮬레이터 실행) — [`iosApp/README.md`](iosApp/README.md)에 배선 문서화. 프레임워크 링크는 위 gradle 태스크로 자동 검증됨.
- **다음 착수 = Next M1(모델·직렬화).**
...
- **M0 · KMP 골격** — ✅ **완료**(green 루프 3축 통과). `shared + androidApp + iosApp`, Koin `startKoin` 배선, 공유 `Greeting`을 양 플랫폼 Compose 화면에 표시.
  ...
  - ⏳ 남음: iOS `iosApp.xcodeproj` 생성(전체 시뮬레이터 실행) — 프레임워크 링크·SKIE는 검증됨.
