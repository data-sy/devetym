...
> | **2** | **웹 트랙 W — 「크게」** | **W0a·W0b·W0c·W1a ✅ 완료** (상세는 [Done](#done--완료)) — 도메인·기반 · 650 정본 D1 적재 · 프록시 하드닝까지 프로덕션 적용됐다 | **▶ 다음 한 걸음 = W1b 웹 본체** — 정적 650장 + 검색 + 상세 + AI 폴백. 사람 게이트 없음. 착수 전 반드시 읽을 것: ① **웹 API를 `devetym.com/api/*`(same-site)로 붙인다** — workers.dev로 직접 부르면 식별 쿠키가 서드파티가 되어 Safari에서 죽고 「브라우저당 3건」 층이 사실상 사라진다 ② **429의 `scope`로 문구를 가른다**(`browser`=앱 유도 / `ip`·`web`=내일 다시) — **Q4 결정(2026-08-25): 한도 화면은 앱 유도를 전면에.** 문구 최종안은 W1b에서 **2~3안 제시 후 사람이 고른다**(살아 있는 사람 결정 지점) ③ **한글 별칭 1,097개를 title·h1·구조화 데이터에 1급으로** — 안 하면 650장은 한국어 검색에 존재하지 않는다. 완료 오라클 = **배포된 실 URL 650개 전수 200** + AI 생성 왕복(로컬 빌드 성공은 오라클이 아니다). 정본 = [`🤖-26-08-25-web-large-track-handoff.md`](🤖-26-08-25-web-large-track-handoff.md) |
>
...
- **🌐 웹(React) 이행 트랙 W — Now에 남은 것은 W1b 하나다.** 유입(acquisition) 목적의 채널 확장이고 **앱은 유지**한다(마이그레이션 아님). 규모는 「크게」(650장 정적 + 웹 AI)로 2026-08-05 사람 확정 — **재론 금지**.
  - **완료분은 [Done](#done--완료)으로 내렸다** — W0a 기반 · W0b 도메인 결선 · W0c 650 D1 시딩 · W1a 프록시 하드닝. 실측값·함정·설계 전제는 **[핸드오프](🤖-26-08-25-web-large-track-handoff.md)가 정본**이다(§4 인용해도 되는 실측값 · §5 다시 밟지 말 것 · §3 하지 말 것).
  ...
  - **마일스톤**: ~~W0(도메인·스캐폴드·토큰 추출)~~ ✅ → ~~W0c(650 D1 시딩)~~ ✅ → ~~W1a(프록시 하드닝)~~ ✅ → **W1b 정적 650 + 검색 + AI 폴백** ← 지금 → **W1c** 승격 잡(= 캐시 M5, `critic` 통과분을 `authored`로 승급해 색인 자격을 연다) → **W2** 카테고리 허브·구조화 데이터·thin content(317개가 300자 미만) → **W3** 8주 실측 리뷰.
