...
- **M8 · 통합·자산·마감 착수 준비** — M7 완료(아래 Done). 다음은 `feat/m8-...`를 `feat/m7-koin-wiring` 위에 **스택** 분기(브랜치 보존). 프로세스: M8 스펙 슬라이스 저작 → 적대 비준 → (게이트 완화) Claude eyes-open 수용 → 구현. **seam actual 실구현**(외관모드 영속·mailto·공유·앱평가·클립보드·기기ID 영속 고유화)·**아이콘/스플래시 자산**(SVG→adaptive/appiconset·런치스크린, `~/dev-etymology/docs/design/icon/` 상속)·**폰트 라이선스 고지**(OFL)·**접근성**·**에러 통합**·**온보딩 영속**·**실 셸 VM 수명주기**(ViewModelStore 도입 → M7 이월 DR5-2 실 창·VM leak 결착). **⚠️ 검증 천장 최대**: seam actual·아이콘 렌더·실 플랫폼 Koin 그래프 완전성·Xcode 빌드·실기기 시각/상호작용은 **실기기/사람 검증 필요** — 코드 완료분과 실기기 필요분을 정직히 분리 라벨.
