...
> | **2** | **웹 트랙 W — 「크게」** | **✅ W0a·W0b 완료 · ADR-0012·0013 ✅비준(2026-08-25) — 사람 게이트 없음** | **W0c 진행 중 (2026-08-26~)** — 격리 환경(로컬 D1 샌드박스·좌표 반전·CI) **완료**, 본체 7단계 **2/7** (§3-1 `normalizeTermKey` = 구분자 삭제·세 지점 동치 · §3-2 `entries.origin` DEFAULT `'generated'`). **다음 한 걸음 = `prompt_version` 센티널 확정** → 이후 authored 650 시딩 · 충돌 규칙 · 익스포트 잡 → **W1a 프록시 하드닝** → **W1b** 650장+검색+AI → **W1c 승격 잡**(2026-08-25 사람 선택 (b) — W 트랙 안에서 닫는다). 정본 = [`🤖-26-08-25-web-large-track-handoff.md`](🤖-26-08-25-web-large-track-handoff.md) |
>
