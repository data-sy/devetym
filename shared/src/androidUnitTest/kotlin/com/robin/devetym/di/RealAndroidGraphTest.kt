 ...
 *
 * 🔒 **부분 스텁 금지(spec §3-1·§4)**: 6개 플랫폼 바인딩 전부 **실 모듈에서** 해석한다. 특히 `DevEtymDatabase`는
...
class RealAndroidGraphTest {
    ...
    private fun realGraph(context: Context) = koinApplication {
        modules(
            ...
            androidPlatformModule(context),                    // ⚠️ 실 모듈(스텁 아님) — 6 바인딩 실 해석
        )
    }.also { app = it }
    ...
    fun test_androidPlatformModule_실그래프_전seam_해석() {
        ...
        // ── 6개 실 플랫폼 바인딩 전부 실 concrete 타입으로 해석(부분 스텁 금지 단언) ──
        ...
        // (6) DeviceInfo — 실: AndroidDeviceInfo(Context)
