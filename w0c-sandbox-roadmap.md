...
> **최종 갱신 2026-09-01 (§3-7 프로덕션 적용 완료 — 본체 7/7. 남은 것은 §8 흡수뿐).** 수명은 브랜치와 같다 — 흡수되면 §8 절차로 접고 이 파일을 지운다.
...
│   │   └── 음성 대조 2종(손편집·정렬 회귀)              ✅
│   └── 7. 원격 적용                        ✅ 2026-09-01 · 프로덕션 라이브
│       ├── 사람 판정 (b) + 1번(배포→시딩→개명)              ✅
│       ├── 프로덕션 백업                                    ✅ 09-01 02:05
│       ├── dev DB 리허설 (프로덕션 클론)                     ✅ 9지표 일치
│       ├── ① 좌표 복원                                      ✅ 5baba3b · main과 설정 차이 0
│       ├── ② 0002 원격 적용 · 21행 백필                      ✅
│       ├── ③ N1 워커 배포                                   ✅ c9ccd526
│       ├── ④ 시딩 2,592문장 · 5,827행 쓰기                   ✅ 3.5s
│       ├── ⑤ 좌초 5건 재정규화 (#21 서버 쪽)                 ✅
│       └── ⑥ 검증 9지표 + 실 앱 왕복 + 익스포트 왕복          ✅ 전부 합격
│
└── 흡수 ─────────────────────────────────── ⬜ ← **여기 · §8 절차**
```

### 다음 한 걸음 = §8 흡수 — 이 문서를 지우고 W1a로 넘긴다

**W0c는 끝났다.** 650이 프로덕션 D1에 있고 실 앱 경로에서 한글 별칭까지 도달한다.
남은 것은 브랜치를 접는 일이다 — 코드 작업이 아니라 정리다.

**§8 절차 (그대로 따르면 된다)**:
1. §7 실측값 중 살아남을 것 → 핸드오프 §4 「인용해도 되는 실측값」
2. §6 함정 중 다시 밟을 수 있는 것 → 핸드오프 §5 「실측으로 얻은 함정」
3. §4 열린 질문 중 미해결(**B — 승격 잡 입력 필터에 `branch` 필요**) → ROADMAP 백로그 또는 Issue
4. ROADMAP Now의 W0c 완료 처리 · 다음 걸음을 **W1a 프록시 하드닝**으로
5. **이 파일을 삭제한다.** 같은 커밋에서 — 남기면 stale 정본이 된다

**병합·푸시는 사람 몫이다**(안전선). 브랜치 2개 = `devetym` · `devetym-proxy` 둘 다.

### 판정 하나 남음 — `devetym-cache-dev`를 지울 것인가

프로덕션 클론 + 시딩·개명이 적용된 상태로 남아 있다(`1be332f5-…`, 무료 3/10).
**유지 근거**: 다음 스키마 변경(W1a·W1c) 때 또 리허설할 곳이 필요하다. 이번에 이게 순서 문제를
잡아냈다. **삭제 근거**: 방치하면 실제와 갈라져 "리허설했다"는 착각의 근거가 된다.
→ **유지하되 W1a 착수 시 프로덕션에서 다시 클론**하는 쪽을 권한다. 클론은 2분이다.

### 완료 상태 (새 세션이 이것만 알면 된다)

| | |
|---|---|
| 프로덕션 | entries **671** · authored **650** · generated **21** · aliases **1,304** · entry_versions 0 |
| 배포 워커 | `devetym-proxy` 버전 `c9ccd526` — N1 정규화 |
| 익스포트 왕복 | 프로덕션 → `terms.json` **바이트 동일** |
| 키 위생 | 1,975개 전부 N1 · 비-N1 **0** |
| 백업 | `~/devetym-d1-backup-20260901-020554.sql` (적용 **전** 상태) |
| 스테이징 | `devetym-cache-dev` (`1be332f5-d3fb-48cb-9ba7-fc481c2a0fec`) |
...
## 3-7d. 프로덕션 적용 결과 — ✅ **완료 2026-09-01**

`accept edits` 모드로 전환하니 좌표 복원 편집이 통과했다(차단은 auto mode 전용 분류기였다).
①~⑥을 순서대로 실행했고 **전부 합격**했다.

| 단계 | 결과 |
|---|---|
| ① 좌표 복원 | `5baba3b` · 주석 제외 `main`과 설정 차이 **0** · `npm test` 83/83 |
| ② `0002` 원격 적용 | 2 commands · 기존 **21행이 `generated`로 백필** |
| ③ N1 워커 배포 | 버전 `c9ccd526` · 바인딩 4개 전부 프로덕션 확인 · 배포 직후 `bash` 200 |
| ④ 시딩 | **2,592 문장 · 5,827행 쓰기 · 3.5초** |
| ⑤ 좌초 5건 재정규화 | 6 문장 · 10행 |
| ⑥ 검증 | 아래 |
...
| 지표 | 값 | | 지표 | 값 |
|---|---|---|---|---|
| entries | **671** ✅ | | entry_versions | 0 ✅ |
| authored | **650** ✅ | | FK dangling | 0 ✅ |
| generated | 21 ✅ | | 축 드리프트 | 0 ✅ |
| aliases | **1,304** ✅ | | 별칭이 엔트리 가림 | 0 ✅ |
| prompt_version 종류 | 2 ✅ | | | |

**실 앱 경로 왕복** (`https://devetym-proxy.data-sy-2.workers.dev`)

| 요청 | 도달 | 태그 |
|---|---|---|
| `AA 트리` · `aa-tree` · `aatree` | `aa-tree` | `authored:` |
| `추상 팩토리` | `abstract-factory` | `authored:` |
| `집계` | `aggregate` | `authored:` |
| `blue green deployment` | `blue-green-deployment` | `authored:` |
| `bash` | `bash` | `v2-pathA:` (generated 무영향) |

**닫힘 증명 둘**
- 프로덕션 → `terms.json` 익스포트가 **커밋된 파일과 바이트 동일** → ADR-0012의
  「익스포트·커밋 의무」가 처음으로 실제로 닫혔다
- 전체 키 **1,975개 중 비-N1 키 0** → 좌초 행이 남지 않았다

⚠️ **entry_versions가 0이라는 건 충돌이 없었다는 뜻이다.** 충돌 규칙(§3-5)은 프로덕션에서
실제로 발화하지 않았다 — 리허설·로컬에서만 검증됐다. 다음 시딩에서 처음 쓰일 수 있다.

---

## 3-7c. 프로덕션 실행 순서 — ~~확정 · 사람이 쳐야 함~~ **✅ 09-01 실행 완료** (기록용)
