...
> **최종 갱신 2026-09-01 (§3-6 익스포트 완료 — 로컬 작업 종료).** 수명은 브랜치와 같다 — 흡수되면 §8 절차로 접고 이 파일을 지운다.
...
│   │   └── 음성 대조 2종(손편집·정렬 회귀)              ✅
│   └── 7. 원격 적용 〔사람 판정〕          ⬜ ← **여기서 멈춘다**
│       └── 좌표 복원 없이는 시도 자체가 실패 · §4-A 선택지 미정
│
└── 흡수 ─────────────────────────────────── ⬜ §8 절차
```

### 다음 한 걸음 = **사람 판정** — 원격 적용을 어떻게 칠 것인가 (§3-7 · §4-A)

**로컬에서 할 수 있는 것은 다 끝났다.** 1~6이 전부 녹색이고, 남은 하나는 코드가 아니라
결정이다. 실 D1에 붓는 순간은 되돌리기 어렵고, 지금 좌표가 반전돼 있어 **복원 없이는
시도 자체가 실패한다**(§1 표).

**Claude가 더 진행하지 않는 이유**: 이건 막힌 게 아니라 사람 몫으로 설계된 지점이다.
ADR-0012로 D1이 정본이 된 뒤 실 D1 쓰기는 캐시 오염이 아니라 **정본 손상**이다.

| 선택지 | 무엇 | 대가 |
|---|---|---|
| **(a)** 사람이 직접 wrangler 실행 | 좌표 복원 후 사람이 명령을 친다 | 사람 시간 · 손으로 치는 실수 |
| **(b)** `devetym-cache-dev`에 먼저 | 별도 DB에 붓고 확인 후 본DB (무료 10개 중 2개 사용) | 단계 하나 추가 · 두 DB 동기화 부담 |
| **(c)** Claude가 좌표 복원 후 실행 | 로컬과 같은 명령을 원격에 | 되돌리기 어려운 쓰기를 자동 실행 |

**현 시점 재료**: 로컬에서 668행·1,301별칭이 실 워커로 서빙되고, 익스포트가 바이트 동일로
닫히며, 충돌 규칙은 심은 충돌에서 검증됐다. 원격에 실제로 있는 것은 generated 18행뿐이고
authored×generated 충돌은 **0건**이라 실 적용에서 밀려날 행이 없다.

**결정 전에 열려 있는 규범 질문 하나**: §4-B — 승격 잡의 입력 필터에 `branch`가 필요하다.
`not_dev_term` 11 · `possible_typo` 1은 페이지가 되면 안 된다. W1c 착수 전 판정이면 되고,
§3-7을 막지는 않는다.
