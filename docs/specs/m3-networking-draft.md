...
- [x] (비준 종료·ESCALATE·2026-07-05 → **구현에서 해소** 2026-07-05) **잔여 Blocker AD-1 — 2xx 비JSON/빈바디의 `NoTransformationFoundException` 미포착**. Ktor `ContentNegotiation`의 `res.body<ClaudeResponse>()`는 응답 `Content-Type`이 `application/json`이 아니거나(Cloudflare 200 챌린지 HTML·빈 200 바디) 디코드 실패를 `SerializationException`이 아니라 `NoTransformationFoundException`으로 던져 `catch (e: SerializationException)`가 놓쳤다. **해소(해소책 (i)+(iii) 채택)**: `ClaudeApi.generate`의 body-디코드 `try` catch를 넓혀 `CancellationException`·`ClaudeException`·`HttpRequestTimeoutException`·`IOException`을 각각 재던지거나 매핑한 뒤 **잔여 모든 `Throwable`을 `InvalidResponse`로 봉인**(코루틴 취소만 보존, `ClaudeApi.kt`). §6-A에 `test_generate_비JSON_contentType_InvalidResponse`(200+text/html)·`test_generate_빈바디_InvalidResponse`(200+빈 바디) canned 2건 추가 — 4축 green(JVM 39·네이티브 31)으로 실측 폐쇄. **게이트 정책 변경(2026-07-05, 메모리 milestone-human-gate-relaxed): 구현 전 사람 비준 게이트 완화 → Claude가 eyes-open 수용하되 이 Blocker는 수용이 아니라 구현으로 닫음.**
- [x] (비준 종료·ESCALATE·2026-07-05 → **eyes-open 수용·구현 완료**) **비준 6라운드(cap=6) 종료, ESCALATE**(Blocker 1=AD-1, 위에서 구현 해소). verdict 로그 정본: `~/dev/agent-harnesses/runs/`(task `wichob0hh`). 게이트 정책 완화 하 사람 게이트를 구현 후 아침 리뷰로 이동 — **잔여 residual(AD-1)은 구현에서 닫혔고, downstream 이월(서버 트랙 INV-13·M4 category 정규화·M8 실엔진 예외매핑)은 각 DoD로 상속됨**. `-draft` 유지(M1·M2와 동일 — 제거 안 함).
