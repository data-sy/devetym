...
> **⚠️ RMW 원자성 가정 (carry-forward, DR-2)**: 위 read-modify-write(`store.selectByKeyword` → `store.upsertTerm`)는 트랜잭션으로 감싸지 않는다 — `LocalTermStore`는 얇은 위임 추상화라 트랜잭션 경계를 두지 않는다. 따라서 M4는 `fetch`/`refresh`/`toggleBookmark`를 **같은 keyword에 대해 단일-writer로 직렬화**된 스코프에서 호출한다고 가정한다(동일 keyword 동시 쓰기 없음). 멀티스레드 스코프에서 같은 keyword의 `refresh`(네트워크 왕복 후 RMW)와 `toggleBookmark`(동기 RMW)가 겹치면 lost-update로 북마크·`seenAt`이 조용히 되돌려질 수 있으나, 창이 좁아 SQLDelight `transaction` 원자화는 후속으로 이월한다(Open Questions).
...
- [ ] (선상속·DR-2) RMW 원자성 — `fetch`/`refresh`/`toggleBookmark`의 비원자 read-modify-write(`selectByKeyword`→`upsertTerm`, 트랜잭션 없음)를 SQLDelight `transaction`으로 감쌀지, 혹은 소비 계약의 단일-writer 직렬화로 충분한지. M4는 **동일 keyword 단일-writer 직렬화 가정**(§3-4)으로 진행하고 트랜잭션 하드닝을 이월.
...
- [ ] (선상속·드문 엣지·DR-4) 크로스버전 번들 제거 시 `source=BUNDLE` 북마크 로우의 AI 승격 정책 — 번들 북마크(`source=BUNDLE`, `seenAt=null`) 용어가 다음 버전 번들에서 빠지면 network `Found(AI)` upsert가 `source=BUNDLE` 보존(§3-4)이라 이후 매 fetch마다 3단 캐시 불인정→네트워크 재호출(조용한 캐시 우회, 데이터 손상 아님). network Found upsert에서 `source=BUNDLE`→`AI` 승격할지 정책 결정을 이월.
