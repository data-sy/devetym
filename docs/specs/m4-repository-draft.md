...
- **⚠️ 전제조건 — 단일-writer 직렬화(DR-2)**: 소비자(M5)는 **같은 keyword에 대한 `fetch`/`refresh`/`toggleBookmark`를 직렬화**해 동시 실행하지 않는다. `refresh`는 네트워크 왕복(수 초) 동안 RMW 창이 열려 있어, 같은 keyword의 동시 쓰기는 lost-update(북마크·`seenAt` 조용한 되돌림)를 낳는다(§3-4). M4 내부 가정이 아니라 **유일 인터페이스인 이 지점에 명시해 M5로 전파하는 계약**이다.
...
> **⚠️ RMW 원자성 — 단일-writer 계약(DR-2)**: 위 read-modify-write(`store.selectByKeyword` → `store.upsertTerm`)는 트랜잭션으로 감싸지 않는다 — `LocalTermStore`는 얇은 위임 추상화라 트랜잭션 경계를 두지 않는다. 그래서 **같은 keyword에 대한 `fetch`/`refresh`/`toggleBookmark`의 단일-writer 직렬화(동일 keyword 동시 쓰기 없음)를 `TermRepository` 인터페이스의 명시 전제조건으로 둔다**(§3-1) — M4 내부 가정이 아니라 유일 소비자 M5가 지켜야 하는 계약으로 전파한다. `refresh`의 RMW 창은 **네트워크 왕복 전체(수 초)라 좁지 않다**: 같은 keyword의 `refresh`(옛 로우를 읽고 네트워크 대기)와 `toggleBookmark`(동기 RMW)가 겹치면 refresh가 옛 값으로 덮어 북마크·`seenAt`이 조용히 되돌려진다. lost-update가 실재하므로 **'창이 좁다'는 이전 정당화는 refresh에 대해 철회한다** — 방어는 계약(직렬화)으로 하고, 계약 대신 SQLDelight `transaction`으로 원자화할지는 후속 하드닝으로 이월한다(Open Questions).
...
- [ ] (선상속·DR-2) RMW 원자성 — `fetch`/`refresh`/`toggleBookmark`의 비원자 read-modify-write(`selectByKeyword`→`upsertTerm`, 트랜잭션 없음). M4는 **단일-writer 직렬화를 `TermRepository` 인터페이스 전제조건으로 명시 전파**(§3-1·§3-4)해 진행한다(M4 내부 가정 아님 — 유일 소비자 M5가 지킬 계약). `refresh`의 RMW 창은 네트워크 왕복 전체라 좁지 않으므로 이전 '창이 좁다' 정당화는 철회했다. 계약 대신 SQLDelight `transaction` 원자화(계약 없이도 안전)로 대체할지는 하드닝으로 이월.
