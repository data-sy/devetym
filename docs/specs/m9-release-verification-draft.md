...
- **`[AI]` 부분 스텁 금지(Blocker 방어 — 필수)**: 실 `androidPlatformModule`의 **6개 바인딩 전부**(`DevEtymDatabase`·`DeviceIdProvider`·`AppActions`·`AppearanceStore`·`OnboardingStore`·`DeviceInfo` — `PlatformModule.android.kt:23-28`)를 **실 모듈에서 해석**해야 한다. 특히 `single<DevEtymDatabase> { createDatabase(DriverFactory(context)) }`(23행)를 Robolectric에서 못 열더라도 **`KoinGraphTest.kt:38,47`의 in-memory JDBC 스텁(`JdbcSqliteDriver.IN_MEMORY` → `testDatabase()`) 패턴으로 되돌리는 것을 금지**한다 — 그 순간 6 바인딩 중 누락·오배선 위험이 가장 큰 DB/드라이버가 스텁인 채 "실 그래프 완전성 폐쇄" 거짓 green이 뜨고 DR-1이 경고한 마스킹이 재발한다(복붙 가능한 스텁 패턴이 이미 코드에 실재). **어느 바인딩이든 Robolectric서 실 해석이 불가하면 스텁으로 대체하지 말고 그 항목만 `[사람]` 잔여로 명시 강등**한다. **단 6 바인딩은 DR-1 주장에 동급이 아니다 — DB/드라이버(`DevEtymDatabase`)는 DR-1이 겨눈 하중 바인딩이라 명시 강등 예외에서도 제외한다: DB가 실 해석되지 않으면(Robolectric SQLite가 `Schema.create` 실패 등) §3-1은 "DR-1 Android 절반 폐쇄" green을 주장할 수 없고, `[AI→사람]` 정적 대조(§7 Q1 폴백 등급) + `[사람]` 잔여로 격하된다.** DB를 `[사람]`으로 "드롭"해 나머지 5 바인딩만 green 내는 것은 스텁 대체와 등가(둘 다 하중 바인딩 미검증)이므로 금지. 테스트는 **각 바인딩이 실/스텁 중 무엇인지 단언·주석으로 표기**해 DB 강등이 "절반 폐쇄" green 아래로 조용히 통과하지 못하게 한다.
...
- **`[AI]` 처방(로직만)**: OS 핸드오프와 **분리 가능한 순수 로직**을 계약 테스트로 닫는다 — (i) `AppearanceStore.set(mode)` → `MutableStateFlow` 방출값 = 입력 mode(0/1/2 클램프 포함) 단언, (ii) 외관 매핑 `mode→dark`(0=시스템/1=false/2=true) 순수 함수 테스트, (iii) Android `AndroidAppActions`의 Intent **구성**(action=`ACTION_SENDTO`·data scheme=`mailto`·`ACTION_SEND` type=`text/plain`) Robolectric `Shadow` 단언, (iv) 클립보드 쓰기 후 `ShadowClipboardManager` 값 단언, **(v) iOS 외관 키부재 기본다크** — `iosSimulatorArm64Test`에서 NSUserDefaults `appearance_mode` 키 clear 후 `UserDefaultsAppearanceStore().mode.value == 2`를 단언한다(`IosSeams.kt:56-59`의 `objectForKey==null → 2` 분기; `integerForKey`가 키 부재 시 0=시스템을 반환하는 함정이라 신규 iOS 설치 첫 실행이 다크가 아니라 시스템으로 **에러 없이 조용히 발산**하는 최고위험 seam 로직 — 실기기에서도 눈에 안 띌 수 있어 `[AI]`가 사람 없이 닫는다). **로직·구성은 AI가 닫고, 실제 앱 열림·시트 표시·픽셀 전환만 사람으로 남긴다.**
...
- **부분 스텁 금지**(Blocker 방어·§3-1): 실 그래프 완전성 테스트는 6개 실 플랫폼 바인딩 중 어느 하나도 스텁(특히 DB in-memory JDBC)으로 대체하지 않는다 — 못 여는 항목은 스텁이 아니라 `[사람]` 잔여로 강등하고, 각 바인딩의 실/스텁 여부를 표기한다. 표기 없는 그래프 green 금지(DR-1 마스킹 재발 차단).
...
- **`[AI]` green(자동으로 닫는 부분)**:
  - §3-1 실 `androidPlatformModule` 그래프 완전성 테스트 green(Android DR-1 절반 폐쇄) — **6 바인딩 전부 실 해석(부분 스텁 금지). DB/드라이버 실 해석은 "절반 폐쇄" green의 하드 전제 — DB 강등 시 green 미충족, `[AI→사람]` 정적 대조로 격하(§7 Q1 폴백). 그 외 못 여는 항목만 `[사람]` 명시 강등, 각 바인딩 실/스텁 표기 포함**.
  - §3-2 `NativeSqliteDriver` 네이티브 DB 왕복 테스트 green(B1 잔여 절반 폐쇄).
  - §3-3 seam 로직 계약 테스트 green(외관 전파·Intent 구성·클립보드 로직 + **iOS 키부재 기본다크 `mode==2`**).
  - §3-4 라이선스 로드 테스트·§3-5 대비비/커버리지 리포트 산출.
  - 기존 3축(`:shared:testDebugUnitTest`·`:androidApp:assembleDebug`·`:shared:linkDebugFrameworkIosSimulatorArm64`) green **유지(회귀 0)** + **신규 `:shared:iosSimulatorArm64Test` 축 신설·green 달성**. ⚠️ 이 축은 현재 미구성이다(`shared/build.gradle.kts`에 `commonTest`·`androidUnitTest`만; line 61 "네이티브엔 JDBC 없음") — §3-2·§3-3(v)가 여기 착지하므로 소스셋+테스트 의존 배선 신설이 M9 신규 좌표(§7 Q6·메모리 `green-oracle-native-execution-gap` B1). "회귀 0 유지"가 아니라 "신설 후 달성"임을 정직히 구분.
...
- `test_androidPlatformModule_실그래프_전seam_해석` — 실 모듈(스텁 아님) + 모의/Robolectric Context로 `KoinAppDependencies` 전 seam `get()` 해석 green. **6개 실 바인딩 전부 실 모듈서 해석(부분 스텁 금지) — DB를 in-memory JDBC로 되돌리지 않고, 각 바인딩 실/스텁을 단언·주석 표기**(§3-1).
...
- `test_ios_외관_키부재_기본다크` — `iosSimulatorArm64Test`에서 NSUserDefaults `appearance_mode` 키 clear 후 `UserDefaultsAppearanceStore().mode.value == 2`(신규 iOS 첫 실행 다크 기본, `IosSeams.kt:56-59` `objectForKey==null` 분기)(§3-3, `[AI]`).
...
1. **Robolectric 재사용 vs 정적 대조(§3-1) — 단, §3-3과 결합 판정** — ⚠️ Robolectric은 이미 **§3-3(iii)(iv)가 하드 의존**한다: `AndroidSeams.kt`의 `Intent(...)`·`ClipboardManager`는 순수 JVM `testDebugUnitTest`에서 `RuntimeException("… not mocked")`으로 즉사하므로, Intent 구성·클립보드 계약 테스트는 Robolectric 없이는 성립 불가(도입 비용은 §3-3에서 이미 지불). 따라서 결정 축은 "Robolectric **도입 여부**"가 아니라 "**§3-1도 그 Robolectric을 재사용**할지 vs 신규 좌표 최소화 위해 실 모듈 정의 vs 요구 타입 **정적 대조표**(`[AI→사람]`)로 §3-1만 폴백"할지다. 제안: §3-3이 이미 Robolectric를 세우니 §3-1도 재사용(끌어내리기 최대화). 만약 정적 대조 폴백을 택하면 **§3-3(iii)(iv)의 `[AI]` 태그·DoD도 함께 재판정**해야 한다(둘의 판정을 분리하지 말 것 — §3-3만 Robolectric green이고 §3-1은 정적, 또는 그 역이 되지 않게). 비준 판정.
...
6. **`iosSimulatorArm64Test` 소스셋 신설(§3-2·§3-3(v)의 착지 축)** — §3-2 네이티브 DB 왕복·§3-3(v) iOS 키부재 다크 테스트가 착지할 `iosSimulatorArm64Test` 소스셋이 **현재 미구성**이다(`shared/build.gradle.kts:29-66`에 `commonTest`·`androidUnitTest`만; line 61 "네이티브엔 JDBC 없음(§7-4)"). 즉 §3-2·§3-3(v)의 `[AI]` 끌어내리기는 **없는 축에 착지**하므로, 이 축·`NativeSqliteDriver` 테스트 의존 배선 신설이 M9 신규 좌표다(B1 잔여 절반을 실제로 닫으려면 필수). 이는 링크만 되던 네이티브를 **실행 축**으로 올리는 것 — 메모리 `green-oracle-native-execution-gap`이 마일스톤마다 재발한다고 예고한 클래스. 제안: 신설(끌어내리기 완수), 신규 좌표라 §7 Q1 Robolectric과 같은 등급의 비준 판정 대상. 신설 못 하면 §3-2·§3-3(v)는 `[사람]`/실기기로 정직 이관.
