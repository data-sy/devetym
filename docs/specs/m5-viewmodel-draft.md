...
- [x] (비준 종료·ESCALATE → **eyes-open 수용·구현 완료** 2026-07-05) §7 열린 질문 1~5 판정: **OQ-1(ViewModel base) 실측 확정** — `androidx.lifecycle.ViewModel`(`org.jetbrains.androidx.lifecycle:lifecycle-viewmodel:2.9.6`) + `viewModelScope`가 iosSimulatorArm64 klib로 소비되고 `Dispatchers.setMain`+`runTest`로 네이티브 실행 green(20건). **koin-compose-viewmodel 좌표 불필요**(생성자 주입 형태만 — Koin 배선은 M7). OQ-2(디바운스 ViewModel 배치)·OQ-4(Loading 최소상태)·OQ-5(commit trim만)은 구현대로 수용. **OQ-3(단일-writer 강제)은 미해소·이월**(§3-5·아래) — M5는 계약을 계승하되 Mutex를 코드로 넣지 않고 제안으로만 두며 강제 실측을 M7/하드닝으로 넘긴다(정직한 이월). ESCALATE 최종 잔여 **DR6-2**(load/refresh 단일 취소 슬롯)는 구현에서 `currentJob` 하나를 공유해 해소, **DR5-1**(toggleBookmark guard)·**DR5-4**(취소≠Error)는 §6 discriminating 테스트로 방어. verdict 로그: task `w1qm3mez3`.
