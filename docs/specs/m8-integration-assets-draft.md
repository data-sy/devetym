...
- **실기기 스모크 체크리스트(핸드오프 명항목)**: 「첫 기동 온보딩 게이트 = 실 `androidPlatformModule`/`iosPlatformModule`의 `OnboardingStore` 바인딩 해석」을 포함한다(M7 open q의 실-플랫폼 그래프 스모크 계열에 편입). 실 바인딩 누락 시 첫 기동 크래시는 그래프 테스트 스텁으로 마스킹돼 green이 못 본다 → 실기기 첫 기동으로만 확인된다.
