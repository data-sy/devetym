...
- **외관 배선**(`AppRoot`가 `appearance.mode`를 소비해 실제 테마 결정): 순수 commonMain Compose 변경이라 `assembleDebug`/link로 검증 가능(실기기 천장 아님). 값이 저장만 되고 테마에 반영 안 되는 inert 상태를 방지(§3-6).
...
- **seam actual 런타임 동작**(mailto 앱 열림·공유 시트·평가 다이얼로그·클립보드): 코드 완료·컴파일 보증, **동작은 실기기**. ※ 외관은 **배선(§3-6)까지 코드 완료·`assembleDebug` 검증 대상**이며, 실기기 천장에 남는 것은 **최종 픽셀 렌더 충실도**뿐(다크/라이트 전환의 실제 화면 표시). 배선 자체를 실기기 뒤에 숨기지 않는다.
...
- **검증**: 순수 commonMain Compose 변경 → `:androidApp:assembleDebug` + `:shared:linkDebugFrameworkIosSimulatorArm64`로 배선 완결 검증(실기기 천장 아님). 최종 화면 픽셀 렌더만 실기기.
