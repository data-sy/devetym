...
> **시스템 프롬프트·도구 스키마는 `commonMain`에 산다.** 프롬프트 엔지니어링이 클라이언트 책임이라는 현재 계약을 그대로 옮긴다(→ 향후 서버로 옮길지는 [ADR 후보](adr/)).
...
2. **모델·직렬화** — 어원 항목을 `@Serializable data class`로.
3. **로컬 DB** — 스키마·마이그레이션 정리(SQLDelight/Room 확정 후).
4. **네트워킹** — Ktor 클라이언트 + Claude 요청/응답(tool_use 파싱). 프록시 계약 그대로.
5. **Repository** — `fetch` 3단 흐름 + 북마크/히스토리.
...
| 프록시 경계·프롬프트 위치 | ✅ [ADR-0004](adr/0004-backend-proxy-boundary.md) | 계약 계승, 프롬프트는 클라이언트(`commonMain`) |
| iOS interop: SKIE vs Swift Export | ⏳ ADR 예정 | 버전 민감 — 착수 시점에 작성 |
