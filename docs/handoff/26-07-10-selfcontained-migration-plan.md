...
> # 🟡 거의 소진 — WU-7만 남았다 (2026-08-19)
>
> WU-1~6·8~10 **전건 완료**(이관·자기완결성 확증·코드 갭 3건). **잔여 = WU-7(원본 repo `~/dev-etymology` 폐기) 단 하나이며 사람 게이트다.**
> 그 외 항목은 실행 지시가 아니라 **이력**이다 — 다시 실행하지 말 것. 진행 상태 정본은 [`ROADMAP.md`](../../ROADMAP.md).
...
**WU-12 · 코드 서명·심사·스토어 게시 `[사람/외부]`** — ⚠️ **출시 순서 확정(2026-07-13): iOS 먼저·Android 나중** → 아래 두 트랙으로 분리(하나의 투두로 묶지 않음).
- **WU-12a · iOS 첫 배포 `[사람/외부]`(우선)** — App Store Connect·Xcode 아카이브·서명·심사 직행. 사용자 iOS 출시 경험 + Apple Developer Program 결제 완료 → **코호트 게이트 없음**, A~D(public·Pages·실기기·스크린샷) 선행 후 곧바로 제출.
- **WU-12b · Android 첫 배포 `[사람/외부]`(후행)** — Play Console·AAB·keystore + #2 폐쇄테스트 게이트(20명×14일 연속). 코호트 리드타임(≥14일)으로 iOS보다 구조적으로 늦음. iOS와 독립 투두로 병행 착수 가능, 게시만 후행.
