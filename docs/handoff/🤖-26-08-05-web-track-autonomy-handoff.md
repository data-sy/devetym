...
>
> **⚠️ 선행 게이트 (2026-08-05 추가)**: 사람이 **설계 결정 감사**를 발의했다 — [`🤖-26-08-05-web-design-decision-audit.md`](🤖-26-08-05-web-design-decision-audit.md).
> **그 감사의 원장(`26-08-05-web-design-confirmation-ledger.md`)이 "그대로 착수"로 닫히기 전에는 이 문서를 실행하지 않는다.** 원장이 없거나 미완이면 **멈추고 사람에게 보고**한다. 감사가 결정을 뒤집었다면 이 문서의 §3이 먼저 갱신돼야 한다.
>
...
⚠️ **설계 결정은 전부 끝났다.** 결정 **D1~D12**(D11 계측 · D12 폰트는 2026-08-05 스택 재점검에서 추가) · 열린 질문 Q1·Q3·Q5 · 설계 공백 X1~X4가 근거와 함께 닫혀 있다. **재론하지 말 것.** 구현 중 설계가 틀렸다고 판단되면 **고치지 말고 §6 규율대로 사람에게 보고**한다.
...
| **H9** | **CF Web Analytics 비콘 토큰**(D11) — 대시보드에서 사이트 추가 후 발급되는 공개 토큰. 또는 H2 토큰에 Web Analytics 스코프 부여 | 사람이 준 토큰 문자열이 있는지 확인. **없으면 W0-b는 진행하되 스니펫 삽입만 보류하고 보고** — 배포를 막지 않는다 |
...
**부분 착수 규칙**: H1·H2만 있으면 **W0까지는 진행 가능**하다. W1a는 H4~H7 전부, W1b는 H8까지 필요하다. **H9는 어느 단계도 막지 않는다** — 없으면 스니펫만 빠지고 나머지는 그대로 간다(다만 빠진 기간만큼 K2 기준선에 공백이 생기므로 반드시 보고한다). 없는 게 있으면 **가능한 데까지 하고 멈춰 보고**한다 — 추측으로 메우지 않는다.
...
11. **계측 = CF Web Analytics + `usage_log` 표면 태그**(D11, 설계서 §2-1). GA4 기각(동의 배너 → 유입 마찰 0 원칙 위반). **K3의 대부분은 W1a의 표면 태그에서 서버측으로 떨어진다** — 클라에 심는 계측 이벤트는 **앱 링크 클릭 하나뿐**이다.
12. **폰트: 한글은 시스템 폰트 스택, 한글 웹폰트 미채택. 라틴 DM만 Astro 6 Fonts API로 서브셋+WOFF2**(D12, 설계서 §2-2). **DM 서체에는 한글 글리프가 없다** — TTF 7파일을 그대로 인라인하면 `h1`·`title`(전부 한글)은 타이포를 하나도 못 받는다.
13. **PWA(manifest·서비스 워커·설치 가능성)는 웹 v1 제외**(설계서 §5-4). ROADMAP이 안드로이드 보류 근거로 PWA를 들고 있었으나 어느 설계 문서에도 없던 산출물이라 **ROADMAP 쪽 서술을 정정**했다. 서비스 워커는 SSG + 주기 승격과 맞물려 stale 페이지라는 **F6의 새 얼굴**을 만든다.
...
3. **폰트 (D12 — 종전 "TTF 7파일 인라인"에서 개정)**
   - **라틴**: `composeResources/font`의 DM 7파일(OFL)을 **Astro 6 내장 Fonts API**로 서브셋 + WOFF2 변환. 라이선스 동일하므로 파일 자체는 그대로 사용. **TTF 원본(448KB)을 `@font-face`로 직접 싣지 않는다.**
   - **한글**: 시스템 폰트 스택 — `-apple-system, "Apple SD Gothic Neo", "Noto Sans KR", "Malgun Gothic", sans-serif`. **한글 웹폰트를 추가하지 않는다**(서브셋해도 수백 KB~1MB, LCP 파괴).
   - `AppTypography.kt`에서 상속하는 것은 서체 파일이 아니라 **크기·굵기·행간 스케일**이며, 그건 한글에도 적용된다.
4. **CF Web Analytics 스니펫 삽입**(D11) — W0에 넣는 이유는 **K2의 기준선이 배포 시점부터 쌓여야** 하기 때문이다. 나중에 붙이면 초기 구간이 영구 공백이 된다.
5. **빈 사이트 배포** → `devetym.com`이 200 응답.
...
7. **usage 로그에 표면 태그 추가** — F5(앱 429/402 발생률 상승) 관측 장치. **현재 이걸 보는 수단이 없다.** ⚠️ **이 산출물은 동시에 K3의 정본 계기다**(D11) — F5 관측용으로만 만들지 말고, `표면`(web/app) × `결과`(성공/429/402) × `cache_hit`이 집계 가능한 형태로 남긴다. 「세션당 신규 단어 검색」과 「한도 화면 도달 수」가 여기서 나온다.
...
4. **AI 폴백 섬(React)** — 3건 한도 · `ErrorKind` 6종 한글 매핑(**`else` 없이 전수**) · `TermResult` 3분기 · **한도 도달 시 Q4 문구 + 앱 유도**. 앱 유도 링크에 **CF Web Analytics 커스텀 이벤트 1개**를 붙인다(D11) — 웹 v1이 클라에 심는 **유일한** 계측 이벤트이고, §4-6이 설계한 전환 깔때기의 마지막 관측점이다.
...
- [ ] **§1 H배치 9건 검증** — 없는 게 있으면 어디까지 가능한지 판정하고 보고 (**H9는 어느 것도 막지 않는다**)
- [ ] §4 명령 실행 + 검증 4줄 통과
- [ ] 설계서 통독 — 특히 **§2 결정표(D11·D12 포함) · §2-1 계측 · §2-2 폰트 · §3-1 숨은 규칙 10개 · §4-5 캡 3층 · §5-4 PWA 제외 · §9 전체**
