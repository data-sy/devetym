...
- **다음 실행**: ① 심사 노트 콘솔 붙여넣기(s2, 메타 초안 §5에서 복붙) ② main 기준 재아카이브 → 재업로드(대시보드 b6 — 수출 규정 문답은 Info.plist 키로 생략, 빌드 번호는 1차 업로드가 검증 단계에서 거부돼 재사용 가능, 콘솔서 충돌 시 +1) ③ 심사 제출(s4, 사람 지시로만).
