...
>
> **2026-07-27 App Store 게시·라이브** ([개발 어원 사전](https://apps.apple.com/kr/app/id6790429958) · Apple ID `6790429958`).
> 제출→승인→게시가 전부 끝났고 **M9 DoD "스토어 게시"가 폐쇄**됐다. 아래 본문의 "현재 = 심사 대기",
> 미체크 실행 항목, 리젝 대응 절차(§3-5)는 전부 **stale**이다.
>
> **현재 상태 → [ROADMAP M9](../../ROADMAP.md)** (진행 상태 정본). 잔여는 씨딩·리뷰 확보(📆 일정)와 F Android 트랙.
> **리젝·핫픽스가 다시 필요해지면** — 이 문서의 절차가 아니라 ROADMAP에서 현행 상태를 먼저 확인할 것.
>
> 〔이력 보존용. 정돈 대상으로 [`26-07-14-doc-pruning-backlog.md`](26-07-14-doc-pruning-backlog.md) §1에 [압축] 후보로 등재돼 있다.〕

---

> **작성**: 2026-07-13 밤 · **갱신**: 2026-07-14 (PR #11 병합·1차 업로드 거부 해소 반영). **진입**: ~~새 세션에서 "docs/handoff/26-07-13-ios-submission-handoff.md 읽고 E 제출 진행하자"~~ (역할 종료).
> **상태 정본**: [ROADMAP M9](../../ROADMAP.md) + [iOS 출시 대시보드](../release/README.md)(`checked` 속성이 정본). 충돌 시 정본 우선.

## 0. 세션 운영 — 전문가는 하린 리드 (패널 재소집 불필요)

D1~D9 결정 세션의 4인 패널은 **판단**용이었고 전건 확정됐다([결정 로그](../release/README.md)). E는 **실행** 구간이라 패널 전체는 과함 — 다음 축소 편성으로 운영한다:

- **🚢 하린(iOS 릴리즈 엔지니어) = 세션 리드.** 페르소나 정의는 [결정 프롬프트 §전문가 패널](../release/README.md) 재사용. 아카이브·코드 서명·수출 규정 신고·버전 정책·콘솔 문항 해석이 영역. 콘솔에서 예상 밖 문항/옵션이 나오면 하린이 옵션+추천으로 즉석 브리핑(웹 최신 확인 후).
...
- **✅ 접근성 감사 종결(2026-07-14)**: VoiceOver 대본 정식 주행(iPhone 13 mini) → 결함 3건 수정(글리프 과낭독·Dynamic Type 알약 줄바꿈·✕ 라벨, 커밋 `f52f380`) → 실기기 재확인 통과. 잔여 = 출시 후 백로그 2건 + TalkBack([감사 대본 §D](../release/README.md)).
...
- **b7(TestFlight 릴리즈 빌드 재확인) 폐기(2026-07-14 사용자 결정)**: 개발 빌드 실 크래시 실증(양 플랫폼)으로 충분 판정 — 게이트에서 제거, 대체 확인은 게시 후 실사용 Sentry 모니터링(p3)이 겸한다.
...
- 시뮬 검증 환경 보존 중(teardown 금지 — [원장](../release/README.md)). 시뮬·실기기의 기존 설치본은 옛 ID(`com.robin.devetym`) — 새 ID는 별개 앱으로 설치되므로 다음 스모크 전 옛 앱 `simctl uninstall` 권장.

## 2. 콘솔 입력물 (전부 확정 — 복붙 가능)

| 입력 | 정본 위치 | 확정 결정 |
|---|---|---|
| 앱 이름·부제 | [메타 초안 부록 A](../release/README.md) | D3: 「개발 어원 사전」 + 「프로그래밍 용어의 유래와 작명 이유」 |
| 키워드 (95자) | 메타 초안 §2 | D4 |
...
| 스크린샷 | raw `~/devetym-shots/ios/` · **프레임드 5컷 × 2사이즈(D2 개정 2026-07-14)**: 6.9" = `framed-6.9/`(1320×2868) · 6.5" = `framed-6.5/`(1242×2688). 순서 확정: f1 후크(bug)→f2 효용(daemon)→f3 AI→f4 북마크→f5 라이트(캡션·근거 = [캡처 핸드오프 §1b](../release/README.md)). 구 6컷은 `_superseded-framed-6cut/`로 개명(오집 방지 — 업로드는 `framed-6.9/`·`framed-6.5/`만) | D1(D2 개정). 잔여 = 콘솔 슬롯별 업로드 |
| 지역 | **한국 단독** | D2 |
...
2. **빌드·서명 `[사람+AI]`** — [서명·업로드 가이드](../release/README.md) 절차. 체크: ⓪ ~~`fix/m9-iphone-only`→main 병합~~ ✅ PR #12 완료(Sentry 배선도 PR #14로 병합 완료) ① **main 기준** 빌드 ② iOS appiconset Xcode 확정(자산 이관됨 — 빌드에서 최종 확인) ③ ~~실 Sentry DSN 주입(Info.plist)~~ ✅ 코드젠 자동 주입(루트 `.env` — 발급·실증 완료, 수동 절차 불요) ④ 버전 0.1.0 / 빌드 번호(CFBundleVersion 2 준비됨) ⑤ 아카이브 → 업로드(수출 규정 문답은 Info.plist 키로 생략됨).
...
| 파일 | 역할 |
|---|---|
| [ios-launch-dashboard.html](../release/README.md) | 실행 체크리스트 정본 |
| [m9-store-metadata-draft.md](../release/README.md) | 콘솔 입력값 정본(§1·§2·§3·§5) |
| [m9-signing-upload-guide.md](../release/README.md) | 서명·아카이브·업로드 절차 |
| [ios-launch-decision-prompt.md](../release/README.md) | D1~D9 결정 로그 + 페르소나 정의 |
| [m9-screenshot-caption-jig.html](../release/README.md) | 캡션 수정 시 프레임드 재생성 지그 |
| [LAUNCH-CHECKLIST.md](../release/README.md) | 6카테고리 배경·근거 |
