...
7. 🟡 **No crash reporting or analytics backend (by decision).** Analytics deferred (Blocker #1 → Firebase-later). No Crashlytics/Sentry either → **zero post-launch observability at day one** — you'll learn of crashes only from user reports/store vitals. Not a compliance blocker; a risk decision (see §4, §6).
...
- 🟡 `[H]` **Crash reporting (Crashlytics/Sentry).** None yet. **Tracked as a pre-launch-recommended backlog item** (ROADMAP Later → "[Infra] 크래시 리포팅 도입"). Closing this is what buys launch-order flexibility (Blocker #7). Note: adding it requires a privacy-policy §4 update (diagnostics collection).
