...
7. 🟢/🟡 **Crash reporting added (WU-4, 2026-07-10); iOS activation pending.** Sentry wired via **platform-split seam** (a single commonMain KMP wiring broke `:shared:iosSimulatorArm64Test` — no-cocoapods static framework; see WU-4 ledger). **Android = real** (`sentry-android`, uncaught-handler, CI-green). **iOS = no-op** until Sentry Cocoa (SPM/Swift) is switched on in WU-11 — **required before the iOS store launch** ([iosApp/project.yml](../../iosApp/project.yml) procedure). Privacy policy §2-2 + store labels updated. Analytics still deferred (Blocker #1 → Firebase-later). Blocker #7: Android closed, iOS closes at WU-11. Unified commonMain KMP wiring = post-launch (ROADMAP Later).
...
- 🟢/🟡 **Crash reporting (Sentry) — added WU-4 (2026-07-10).** Platform-split seam: **Android real** (`sentry-android`, CI-green), **iOS no-op** pending Sentry Cocoa (SPM/Swift) activation in WU-11 (required before iOS store launch — [iosApp/project.yml](../../iosApp/project.yml)). Privacy policy §2-2 (diagnostics collection) + store labels updated. Unified commonMain KMP wiring deferred to ROADMAP Later. Blocker #7: Android closed, iOS closes at WU-11.
