...
5. [~] **2~4 모은 백로그 진행** — 안전 이관분만 실행, 나머지는 WU로:
   - [x] `[AI]` pages.yml → `.github/workflows/` 이관. → 실배포 **WU-1 착수·⚠️ 보류(2026-07-10)**: [PR #10](https://github.com/data-sy/devetym/pull/10) 스테이징(main 위 인프라 5파일)했으나 **private+무료 플랜이라 Pages 활성화 불가** → public 전환 or GitHub Pro에 의존(아래 "추가 할 일").
...
- [ ] `[사람]` **리모트 안전 확인 후 public 전환** — private `data-sy/devetym`가 안전하게 올라간 것 확인되면(빌드/자산/시크릿 노출 점검 후) repo를 public으로. ⚠️ 되돌리기 어려움(인덱싱·캐시) → 전환 전 시크릿·`local.properties`·키 노출 스윕 먼저. (ROADMAP 브랜치 전략의 "public 전환은 추후 사람 결정"과 정합.) **↔ WU-1 커플링(2026-07-10 발견)**: 무료 플랜은 private repo Pages를 막으므로, **public 전환이 WU-1(방침 URL) 활성화의 전제**다(대안=GitHub Pro 유료). 즉 이 항목을 하면 WU-1도 열린다.
...
- [ ] GitHub repo 공개 범위 — private / public (ROADMAP "그때 결정"). ⚠️ **이제 WU-1(Pages 방침 URL) 활성화가 여기 걸려 있음** — private 유지 시 Pages는 GitHub Pro(유료) 필요.
